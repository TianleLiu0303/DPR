...
def main():
    ...
    # ======= 模拟两个模态的输入 =======
    B = 2                   # batch size
    N_agent = 5             # agent token 数
    N_map = 10              # map token 数
    dim_agent = 128         # agent 特征维度
    dim_map = 256           # map 特征维度

    # dim_inputs 的顺序要和 inputs tuple 顺序一致
    dim_inputs = (dim_agent, dim_map)

    # ======= 随机输入 =======
    x_agent = torch.randn(B, N_agent, dim_agent)
    x_map = torch.randn(B, N_map, dim_map)

    # ======= mask（全部有效） =======
    mask_agent = torch.ones(B, N_agent, dtype=torch.bool)
    mask_map = torch.ones(B, N_map, dtype=torch.bool)

    # ======= attention distance bias（构造对称矩阵） =======
    total_tokens = N_agent + N_map
    attn_dist = torch.rand(B, total_tokens, total_tokens)
    attn_dist = (attn_dist + attn_dist.transpose(-1, -2)) / 2.0  # 对称化

    # ======= 创建 JointAttention 模块 =======
    attn = JointAttention(
        dim_inputs=dim_inputs,
        dim_head=32,
        heads=4,
        enable_attn_dist=True,
        token_num=total_tokens,   # 注意这里必须 = N_agent + N_map
    )

    # ======= 前向计算 =======
    out_agent, out_map = attn(
        inputs=(x_agent, x_map),
        masks=(mask_agent, mask_map),
        attn_dist=attn_dist
    )

    # ======= 打印输出形状 =======
    print("Input agent shape:", x_agent.shape)
    print("Input map shape:", x_map.shape)
    print("Output agent shape:", out_agent.shape)
    print("Output map shape:", out_map.shape)
